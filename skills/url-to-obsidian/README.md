...
All optional. See [`url_to_obsidian.env.example`](url_to_obsidian.env.example) for the full list: Chrome binary path, DevToolsActivePort path, timezone, etc.
...
都是可选的。完整列表看 [`url_to_obsidian.env.example`](url_to_obsidian.env.example)：Chrome 路径、DevToolsActivePort 路径、时区等。
