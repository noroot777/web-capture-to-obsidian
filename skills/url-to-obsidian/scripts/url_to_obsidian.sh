...
CONFIG_FILE="${URL_TO_OBSIDIAN_CONFIG_FILE:-$SKILL_DIR/url_to_obsidian.env}"
...
if [[ -n "${URL_TO_OBSIDIAN_TARGET_DIR:-}" ]]; then
  TARGET_DIR_CONFIGURED=1
fi

DEVTOOLS_FILE="${URL_TO_OBSIDIAN_DEVTOOLS_FILE:-$HOME/Library/Application Support/Google/Chrome/DevToolsActivePort}"
...
TARGET_DIR="${URL_TO_OBSIDIAN_TARGET_DIR:-$HOME/Obsidian/URL to Obsidian}"
STATE_FILE="${URL_TO_OBSIDIAN_STATE_FILE:-$TARGET_DIR/.url_to_obsidian_state.json}"
DEV_BROWSER_TMP="${URL_TO_OBSIDIAN_DEV_BROWSER_TMP:-$HOME/.dev-browser/tmp}"
CHROME_BIN="${URL_TO_OBSIDIAN_CHROME_BIN:-/Applications/Google Chrome.app/Contents/MacOS/Google Chrome}"
MIN_SUPPORTED_CHROME_MAJOR="${URL_TO_OBSIDIAN_MIN_CHROME_MAJOR:-144}"
URLS_JSON="${URL_TO_OBSIDIAN_URLS_JSON:-$DEV_BROWSER_TMP/url-to-obsidian-urls.json}"
SOURCE_JSON="${URL_TO_OBSIDIAN_SOURCE_JSON:-$DEV_BROWSER_TMP/url-to-obsidian-export.json}"
LLM_OVERRIDES_FILE="${URL_TO_OBSIDIAN_LLM_OVERRIDES_FILE:-$DEV_BROWSER_TMP/url-to-obsidian-llm-overrides.json}"
USE_LLM="${URL_TO_OBSIDIAN_USE_LLM:-1}"
SKIP_EXPORT="${URL_TO_OBSIDIAN_SKIP_EXPORT:-0}"
SKIP_GENERATE="${URL_TO_OBSIDIAN_SKIP_GENERATE:-0}"

export URL_TO_OBSIDIAN_TARGET_DIR="$TARGET_DIR"
export URL_TO_OBSIDIAN_STATE_FILE="$STATE_FILE"
export URL_TO_OBSIDIAN_SOURCE_JSON="$SOURCE_JSON"
export URL_TO_OBSIDIAN_LLM_OVERRIDES_FILE="$LLM_OVERRIDES_FILE"
...
ensure_target_dir_configured() {
  ...
  echo "First-time setup required before using url-to-obsidian." >&2
  echo "Create url_to_obsidian.env from url_to_obsidian.env.example and set URL_TO_OBSIDIAN_TARGET_DIR to your Obsidian notes folder using an absolute path." >&2
  exit 1
}

ensure_absolute_target_dir() {
  if [[ "$TARGET_DIR" != /* ]]; then
    echo "URL_TO_OBSIDIAN_TARGET_DIR must be an absolute path. Current value: $TARGET_DIR" >&2
    echo "Update url_to_obsidian.env and set URL_TO_OBSIDIAN_TARGET_DIR to an absolute path like /Users/you/Obsidian/URL Capture." >&2
    exit 1
  fi
}
...
ensure_codex() {
  ...
  echo "Install Codex first, or run with URL_TO_OBSIDIAN_USE_LLM=0 to skip LLM-generated titles, summaries, and tags." >&2
  exit 1
}
