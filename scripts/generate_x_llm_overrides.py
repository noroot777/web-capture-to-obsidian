...
OVERRIDES_FILE = env_path("X_BOOKMARKS_LLM_OVERRIDES_FILE", "~/.dev-browser/tmp/x-bookmarks-llm-overrides.json")
MODEL = os.environ.get("X_BOOKMARKS_LLM_MODEL", "gpt-5.4").strip() or "gpt-5.4"
BATCH_SIZE = max(1, int(os.environ.get("X_BOOKMARKS_LLM_BATCH_SIZE", "12")))
RETRIES = max(0, int(os.environ.get("X_BOOKMARKS_LLM_RETRIES", "2")))
RESUME = os.environ.get("X_BOOKMARKS_LLM_RESUME", "1") == "1"
...
def main() -> int:
    ...
    if shutil.which("codex") is None:
        print("codex CLI is required when X_BOOKMARKS_USE_LLM=1", file=sys.stderr)
        return 1
    ...
    with tempfile.TemporaryDirectory(prefix="x-bookmarks-codex-") as raw_tmpdir:
